...
// World-Studio.live - Post/Content Routes
...
const formatPost = (post, user = null, currentUserId = null) => {
    return {
        ...
        isLiked: currentUserId ? post.likedBy?.some(id => id.toString() === currentUserId.toString()) : false,
        views: post.views || 0,
        ...
        hasPurchased: currentUserId ? post.purchasedBy?.some(id => id.toString() === currentUserId.toString()) : false,
        isOwner: currentUserId ? post.userId?.toString() === currentUserId.toString() : false,
...
router.get("/", async (req, res) => {
    try {
        ...
        if (free === "true") query.$or = [{ isFree: true }, { price: { $lte: 0 } }];
...
router.get("/:id", async (req, res) => {
    try {
        ...
        // Lock paid content
        if (post.price > 0 && !post.isFree && !canAccessContent(post, currentUserId)) {
...
router.post("/", authMiddleware, async (req, res) => {
    try {
        const {
            title, description, type = "image", category = "General",
            tags = [], fileUrl, fileName, filePublicId, fileSize,
            thumbnail, blurredThumbnail, duration, isFree = true,
            price = 0, isPremium = false, isExclusive = false,
            visibility = "public", allowComments = true, allowDownload = false
        } = req.body;
        ...
        const user = await User.findById(req.userId).select("username avatar isVerified");
        if (!user) {
        ...
        const post = await Post.create({
            ...
            fileUrl, fileName, filePublicId, fileSize,
            thumbnail, blurredThumbnail, duration,
            isFree: isFree && price <= 0,
            price: Math.max(0, price),
            isPremium, isExclusive, visibility,
            allowComments, allowDownload,
            status: "published",
            likes: 0, likedBy: [], views: 0,
            comments: [], shares: 0, saves: 0, purchasedBy: []
        });

        await User.findByIdAndUpdate(req.userId, { $inc: { "stats.totalPosts": 1 } });
...
router.put("/:id", authMiddleware, async (req, res) => {
    try {
        ...
        const { title, description, category, tags, isFree, price, isPremium, visibility, allowComments, allowDownload } = req.body;

        if (title) post.title = title.trim().substring(0, 200);
        if (description !== undefined) post.description = description.trim().substring(0, 2000);
        ...
        res.json({ success: true, message: "Post updated", post: formatPost(post, null, req.userId) });
    } catch (err) {
...
router.delete("/:id", authMiddleware, async (req, res) => {
    try {
        ...
        if (post.userId) {
            await User.findByIdAndUpdate(post.userId, { $inc: { "stats.totalPosts": -1 } });
        }
...
router.post("/:id/like", authMiddleware, async (req, res) => {
    try {
        ...
        const alreadyLiked = post.likedBy.some(id => id.toString() === req.userId.toString());
        ...
        if (alreadyLiked) {
            ...
            // Notify owner
            if (post.userId.toString() !== req.userId.toString()) {
                const liker = await User.findById(req.userId).select("username avatar");
                ...
                const io = req.app.get("io");
                if (io) io.to(`user_${post.userId}`).emit("notification", { type: "like", postId: post._id });
            }
        }

        await post.save();
        res.json({ success: true, liked: !alreadyLiked, likes: post.likes, likedBy: post.likedBy });
    } catch (err) {
...
router.post("/:id/comment", authMiddleware, async (req, res) => {
    try {
        ...
        const post = await Post.findById(req.params.id);
        if (!post) return res.status(404).json({ success: false, error: "Post not found" });
        if (!post.allowComments) return res.status(403).json({ success: false, error: "Comments disabled" });
...
router.post("/:id/purchase", authMiddleware, async (req, res) => {
    try {
        ...
        if (post.isFree || !post.price || post.price <= 0) {
            return res.status(400).json({ success: false, error: "This content is free" });
        }

        if (post.purchasedBy.some(id => id.toString() === req.userId.toString())) {
            return res.status(400).json({ success: false, error: "Already purchased" });
        }

        if (post.userId.toString() === req.userId.toString()) {
            return res.status(400).json({ success: false, error: "Cannot purchase own content" });
        }
        ...
        const platformFee = Math.floor(post.price * 0.15);
        const creatorShare = post.price - platformFee;

        // Deduct from buyer
        buyer.wallet.balance -= post.price;
        buyer.wallet.totalSpent = (buyer.wallet.totalSpent || 0) + post.price;
        ...
        if (creator) {
            creator.wallet = creator.wallet || { balance: 0, transactions: [] };
            creator.wallet.balance += creatorShare;
            creator.wallet.totalReceived = (creator.wallet.totalReceived || 0) + creatorShare;
            ...
            if (creator.wallet.transactions.length > 500) {
                creator.wallet.transactions = creator.wallet.transactions.slice(0, 500);
            }
            ...
            creator.unreadNotifications = (creator.unreadNotifications || 0) + 1;
            await creator.save();
        }
            ...
            if (PlatformWallet) {
                ...
                if (wallet) {
                    ...
                    wallet.history.unshift({
                        ...
                        reason: `Content sale: ${post.title}`,
                        postId: post._id,
                        date: new Date()
                    });
                    await wallet.save();
        ...
        const io = req.app.get("io");
        if (io) {
            io.to(`user_${post.userId}`).emit("content_sold", { postId: post._id, buyerUsername: buyer.username });
        }

        console.log(`💰 Content purchased: "${post.title}" by ${buyer.username}`);
...
router.post("/:id/report", authMiddleware, async (req, res) => {
    try {
        ...
        if (post.reports.some(r => r.userId.toString() === req.userId.toString())) {
...
router.get("/user/:userId", async (req, res) => {
    try {
        ...
        const query = { userId: safeId(req.params.userId), status: { $ne: "deleted" } };
        if (type && type !== "all") query.type = type;

        const [posts, total, user] = await Promise.all([
            Post.find(query).sort({ createdAt: -1 }).skip(parseInt(skip)).limit(parseInt(limit)).lean(),
            Post.countDocuments(query),
            User.findById(req.params.userId).select("username avatar isVerified").lean()
        ]);
...
router.get("/saved", authMiddleware, async (req, res) => {
    try {
        ...
        const userIds = [...new Set(posts.map(p => p.userId?.toString()).filter(Boolean))];
        const users = await User.find({ _id: { $in: userIds } }).select("username avatar isVerified").lean();
        const userMap = {};
        users.forEach(u => { userMap[u._id.toString()] = u; });

        const formatted = posts.map(post => formatPost(post, userMap[post.userId?.toString()], req.userId));
...
router.get("/trending", async (req, res) => {
    try {
        ...
        const formatted = posts.map(post => formatPost(post, userMap[post.userId?.toString()]));
