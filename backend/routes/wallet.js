...
// World-Studio.live - Wallet Routes (UNIVERSE EDITION 🚀)
...
const FRONTEND_URL =
    process.env.FRONTEND_URL || "https://world-studio.live";
...
router.post("/verify-payment", authMiddleware, async (req, res) => {
    try {
        ...
        const alreadyProcessed = user.wallet.transactions.some(
        ...
        if (coins <= 0) {
            return res
                .status(400)
                .json({ success: false, error: "Invalid coins" });
        }

        const pkg = COIN_PACKAGES.find(
    ...
    async (req, res) => {
        ...
        try {
            if (webhookSecret && sig && stripe) {
                ...
                // Fallback: direct parse (e.g. in dev / no secret)
                event =
        ...
        if (event.type === "checkout.session.completed") {
            ...
            try {
                ...
                const coins =
                    parseInt(session.metadata?.coins) || 0;
                ...
                if (userId && coins > 0) {
                    ...
                    if (user) {
                        ...
                        if (!alreadyProcessed) {
                            ...
                            await user.save();
                            console.log(
                                `✅ [WEBHOOK] +${coins} for ${user.username}`
                            );
                        }
                    }
                }
...
router.post("/transfer", authMiddleware, async (req, res) => {
    try {
        ...
        sender.wallet.totalSpent =
            (sender.wallet.totalSpent || 0) + amount;
        ...
        recipient.wallet.totalReceived =
            (recipient.wallet.totalReceived || 0) + amount;
        ...
        recipient.unreadNotifications =
            (recipient.unreadNotifications || 0) + 1;
        ...
        console.log(
            `💸 Transfer: ${amount} from ${sender.username} to ${recipient.username}`
        );
        res.json({
            success: true,
            message: `Sent ${amount} to @${recipient.username}`,
        ...
        res
            .status(500)
            .json({ success: false, error: "Transfer failed" });
    }
});
...
router.post("/withdraw", authMiddleware, async (req, res) => {
    try {
        ...
        if (!amount || amount < MIN_WITHDRAWAL) {
            return res.status(400).json({
                success: false,
                error: `Minimum: ${MIN_WITHDRAWAL} coins`,
            });
        }
        if (amount > MAX_WITHDRAWAL) {
            return res.status(400).json({
                success: false,
                error: `Maximum: ${MAX_WITHDRAWAL} coins`,
            });
        }
        ...
        user.wallet.totalWithdrawn =
            (user.wallet.totalWithdrawn || 0) + amount;
        ...
        res
            .status(500)
            .json({ success: false, error: "Withdrawal failed" });
    }
});
...
router.get("/withdrawals", authMiddleware, async (req, res) => {
    try {
        ...
        res
            .status(500)
            .json({ success: false, error: "Failed to fetch" });
    }
});
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            user.wallet.balance += amount;
            user.wallet.totalReceived =
                (user.wallet.totalReceived || 0) + amount;
            ...
            res.json({
                success: true,
                message: `Added ${amount} to @${user.username}`,
                newBalance: user.wallet.balance,
            });
        } catch (err) {
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            res.json({
                success: true,
                message: `Deducted ${amount} from @${user.username}`,
                newBalance: user.wallet.balance,
            });
        } catch (err) {
    ...
    async (req, res) => {
        try {
            ...
            if (!["completed", "rejected"].includes(status)) {
                return res.status(400).json({
                    success: false,
                    error: "Status: completed or rejected",
                });
            }
            ...
            const tx =
                user.wallet.transactions?.id(transactionId);
            if (
                !tx ||
                tx.type !== "withdrawal" ||
                tx.status !== "pending"
            ...
            if (status === "rejected") {
                ...
                user.wallet.totalWithdrawn = Math.max(
                    0,
                    (user.wallet.totalWithdrawn || 0) - refund
                );
