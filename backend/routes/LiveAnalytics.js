...
// World-Studio.live - Live Stream Analytics Routes
...
const getDateRanges = () => {
    ...
    return { now, todayStart, yesterdayStart, weekStart, monthStart, lastMonthStart, lastMonthEnd };
};
...
router.get("/summary", async (req, res) => {
    try {
        ...
        const recent = await Stream.find()
            ...
            .select("title category viewers isLive startedAt endedAt duration totalGifts streamerName")
            .lean();
        ...
        res.json({
            ...
            live: {
                ...
                avgViewersPerStream: Math.round(viewerStats[0]?.avgViewers || 0)
            },
            streamsToday,
            topCategories,
            recent
...
router.get("/trending", async (req, res) => {
    try {
        ...
        // Trending = high viewers + recent gifts + growing
        const trending = await Stream.find({ isLive: true })
            .sort({ viewers: -1, totalGifts: -1, startedAt: -1 })
            .limit(parseInt(limit))
            .populate("streamerId", "username avatar isVerified followersCount")
...
router.get("/categories", async (req, res) => {
    try {
        ...
        else if (period === "month") dateFilter = { startedAt: { $gte: monthStart } };
        ...
        const categories = categoryStats.map(cat => {
            const live = liveCategories.find(l => l._id === cat.category) || {};
            return {
                ...cat,
...
router.get("/peak-hours", async (req, res) => {
    try {
        ...
        for (let i = 0; i < 24; i++) {
            ...
            allHours.push(existing || { hour: i, streamCount: 0, avgViewers: 0, totalGifts: 0 });
        }

        // Find peak hour
        const peakHour = allHours.reduce((max, h) =>
            h.streamCount > max.streamCount ? h : max, allHours[0]
        );
        ...
        res.json({
            ...
            peakHourFormatted: `${peakHour.hour}:00 - ${peakHour.hour + 1}:00`
        });
    } catch (err) {
...
router.get("/my-stats", auth, async (req, res) => {
    try {
        ...
        const recentStreams = await Stream.find({
            ...
            .sort({ startedAt: -1 })
            .limit(10)
            .select("title category viewers peakViewers duration totalGifts startedAt endedAt isLive")
            .lean();
...
router.get("/stream/:streamId", auth, async (req, res) => {
    try {
        ...
        // Get gift statistics
        let giftStats = { total: 0, count: 0, topGifters: [], giftTypes: [] };
        ...
        if (Gift) {
            try {
                ...
                giftStats.giftTypes = await Gift.aggregate([
                    { $match: { streamId: streamId } },
                    {
                        $group: {
                            _id: "$item",
        ...
        const duration = stream.duration ||
            (stream.endedAt ? Math.floor((new Date(stream.endedAt) - new Date(stream.startedAt)) / 1000) : 0);
        ...
        res.json({
            ...
            stream: {
                ...
                viewers: {
                    ...
                    total: stream.totalUniqueViewers || stream.viewerList?.length || 0,
                    average: stream.avgWatchTime ? Math.round(stream.viewerList?.length * stream.avgWatchTime / duration) : 0
                },
                engagement: {
...
router.get("/admin/overview", auth, requireAdmin, async (req, res) => {
    try {
        ...
        const yesterdayStats = await Stream.aggregate([
            { $match: { startedAt: { $gte: yesterdayStart, $lt: todayStart } } },
            {
                $group: {
                    _id: null,
                    streams: { $sum: 1 },
        ...
        const streamerIds = topStreamers.map(s => s._id).filter(Boolean);
        const streamersInfo = await User.find({ _id: { $in: streamerIds } })
            .select("avatar isVerified")
            .lean();

        const streamerMap = {};
        streamersInfo.forEach(s => { streamerMap[s._id.toString()] = s; });
        ...
        // Calculate growth
        const today = todayStats[0] || { streams: 0, duration: 0, viewers: 0, gifts: 0 };
        const yesterday = yesterdayStats[0] || { streams: 0, duration: 0, viewers: 0, gifts: 0 };

        const growth = {
            streams: yesterday.streams > 0 ? Math.round(((today.streams - yesterday.streams) / yesterday.streams) * 100) : 0,
            viewers: yesterday.viewers > 0 ? Math.round(((today.viewers - yesterday.viewers) / yesterday.viewers) * 100) : 0,
            gifts: yesterday.gifts > 0 ? Math.round(((today.gifts - yesterday.gifts) / yesterday.gifts) * 100) : 0
        };
...
router.get("/admin/realtime", auth, requireAdmin, async (req, res) => {
    try {
        ...
        const liveStreams = await Stream.find({ isLive: true })
            ...
            .limit(20)
            .populate("streamerId", "username avatar isVerified")
            .select("title category viewers peakViewers startedAt totalGifts roomId streamerName")
            .lean();

        // Total viewers
        const totalViewers = liveStreams.reduce((sum, s) => sum + (s.viewers || 0), 0);
...
router.get("/admin/streamers", auth, requireAdmin, async (req, res) => {
    try {
        ...
        const streamersWithInfo = streamers.map(s => ({
            ...
            followersCount: userMap[s.userId?.toString()]?.followersCount || 0,
