// src/components/LiveGiftPanel.jsx - WORLD STUDIO LIVE EDITION 🎁
...
export default function LiveGiftPanel({ streamId, hostId, hostUsername, onGiftSent }) {
    ...
    useEffect(() => {
        ...
        if (stored) {
            try { const user = JSON.parse(stored); setCurrentUser(user); setBalance(user.wallet?.balance || 0); } catch (e) { }
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const handleGift = (gift) => {
            ...
            setTimeout(() => { setRecentGifts(prev => prev.filter(g => g.timestamp !== giftData.timestamp)); }, 5000);
        };
        socket.on("gift_received", handleGift);
        socket.on("gift_sent", handleGift);
        return () => { socket.off("gift_received", handleGift); socket.off("gift_sent", handleGift); };
    }, []);

    const filteredGifts = GIFTS.filter(g => g.category === selectedCategory);

    const sendGift = async (gift) => {
        if (!currentUser) { toast.error("Log in to send gifts"); return; }
        if (!hostId) { toast.error("No host to send gift to"); return; }
        if (balance < gift.price) { toast.error("Not enough coins! 💰"); return; }
        if (gift.price >= 1000 && !showConfirm) { setSelectedGift(gift); setShowConfirm(true); return; }

        setSending(true);
        setShowConfirm(false);

        try {
            await api.post("/api/gifts", { recipientId: hostId, item: gift.name, amount: gift.price, streamId });
            socketRef.current?.emit("gift_sent", {
                streamId, roomId: streamId, senderUsername: currentUser.username, senderAvatar: currentUser.avatar,
                senderId: currentUser._id, recipientId: hostId, recipientUsername: hostUsername,
                item: gift.name, icon: gift.icon, amount: gift.price, color: gift.color, timestamp: Date.now(),
            });
    ...
    const formatPrice = (price) => price >= 1000 ? `${(price / 1000).toFixed(price % 1000 === 0 ? 0 : 1)}K` : price;
    ...
    if (!currentUser) {
        return (
            <div className="p-6 text-center">
                ...
                <a href="/login" className="text-cyan-400 hover:underline text-sm">Login now →</a>
            </div>
        );
    }
        ...
        <div className="relative">
                ...
                <div className="absolute inset-0 z-40 bg-black/80 flex items-center justify-center p-4">
                    <div className="bg-gray-900 rounded-2xl p-6 max-w-sm w-full border border-white/20">
                        <div className="text-center mb-4">
                            ...
                            <p className="text-yellow-400 font-bold text-2xl mt-1">💰 {selectedGift.price.toLocaleString()}</p>
                        </div>
                        <p className="text-white/60 text-center mb-4">Send this gift to <span className="text-cyan-400 font-semibold">{hostUsername}</span>?</p>
                        <div className="flex gap-3">
                            <button onClick={() => { setShowConfirm(false); setSelectedGift(null); }} className="flex-1 py-3 rounded-xl bg-white/10 font-semibold hover:bg-white/20 transition">Cancel</button>
                            <button onClick={() => sendGift(selectedGift)} disabled={sending} className={`flex-1 py-3 rounded-xl font-semibold bg-gradient-to-r ${selectedGift.color} hover:opacity-90 transition disabled:opacity-50`}>{sending ? "Sending..." : "Send Gift"}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex border-b border-white/10 overflow-x-auto scrollbar-hide">
                {GIFT_CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`flex-1 min-w-[80px] py-2.5 px-3 text-xs font-semibold transition whitespace-nowrap ${selectedCategory === cat.id ? "text-cyan-400 border-b-2 border-cyan-400 bg-cyan-400/10" : "text-white/50 hover:text-white/80 hover:bg-white/5"}`}>
                        <span className="block text-lg mb-0.5">{cat.icon}</span>{cat.name}
                    </button>
                ))}
            </div>
            ...
            <div className="p-3 grid grid-cols-5 gap-2 max-h-[200px] overflow-y-auto scrollbar-thin">
                {filteredGifts.map((gift) => {
                    ...
                    return (
                        <button key={gift.id} onClick={() => sendGift(gift)} disabled={!canAfford || sending} className={`relative p-2 rounded-xl transition-all duration-200 flex flex-col items-center group ${canAfford ? `bg-gradient-to-br ${gift.color} bg-opacity-20 hover:scale-105 hover:shadow-lg active:scale-95` : "bg-white/5 opacity-40 cursor-not-allowed"}`}>
                            <span className={`text-2xl transition-transform ${canAfford ? "group-hover:scale-110" : ""}`}>{gift.icon}</span>
                            <span className="text-[10px] text-white/80 truncate w-full text-center mt-1">{gift.name}</span>
                            <span className="text-[10px] text-yellow-400 font-bold">{formatPrice(gift.price)}</span>
                            {gift.category === "epic" && <span className="absolute -top-1 -right-1 text-[8px] bg-purple-500 px-1 rounded animate-pulse">✨</span>}
                            {gift.category === "luxury" && <span className="absolute -top-1 -right-1 text-[8px] bg-yellow-500 text-black px-1 rounded font-bold">VIP</span>}
                        </button>
                    );
                })}
            </div>
            ...
            {recentGifts.length > 0 && (
                <div className="p-2 border-t border-white/10 space-y-1 max-h-[100px] overflow-y-auto">
                    ...
                    {recentGifts.map((gift, i) => (
                        <div key={gift.timestamp || i} className={`flex items-center gap-2 bg-gradient-to-r ${gift.color || "from-purple-500/20 to-pink-500/20"} rounded-lg px-2 py-1.5 animate-slideIn`}>
                            <span className="text-lg">{gift.icon}</span>
                            <span className="text-xs truncate flex-1"><span className="text-cyan-400 font-semibold">{gift.senderUsername}</span><span className="text-white/40"> → </span><span className="text-yellow-400 font-medium">{gift.item}</span></span>
            ...
            <p className="text-white/30 text-[10px] text-center py-2 border-t border-white/10">Tap a gift to send to {hostUsername || "host"} • Gifts over 1K need confirmation</p>

            <style>{`.scrollbar-hide::-webkit-scrollbar { display: none; } .scrollbar-thin::-webkit-scrollbar { width: 4px; } .scrollbar-thin::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.2); border-radius: 2px; } @keyframes slideIn { from { opacity: 0; transform: translateX(-10px); } to { opacity: 1; transform: translateX(0); } } .animate-slideIn { animation: slideIn 0.3s ease-out; }`}</style>
        </div>
    );
}