...
export default function WalletPage() {
    ...
    const [wallet, setWallet] = useState({ balance: 0, totalReceived: 0, totalSpent: 0 });
    ...
    const fetchWallet = useCallback(async () => {
        try {
            const [walletRes, transactionsRes] = await Promise.all([
                api.get("/api/wallet"),
                api.get("/api/wallet/transactions").catch(() => ({ data: [] })),
            ]);
    ...
    useEffect(() => {
        const verifyPayment = async () => {
            ...
            // Handle successful payment - VERIFY IT!
            if (success === "true" && sessionId) {
                ...
                try {
                    ...
                    const response = await api.post("/api/wallet/verify-payment", { sessionId });

                    toast.dismiss(toastId);

                    if (response.data.success) {
                        if (response.data.alreadyProcessed) {
                            toast.success("✅ Payment was already processed!", { duration: 4000 });
                        } else {
                            toast.success(
                                <div className="flex flex-col">
                                    <span className="font-bold text-lg">🎉 Payment Successful!</span>
                                    <span className="text-green-300">+{response.data.coinsAdded} coins added</span>
                                    <span className="text-sm opacity-80">New balance: {response.data.balance} coins</span>
                                </div>,
                                { duration: 6000 }
                            );
                        }
                    ...
                    if (errorMessage.includes("already processed")) {
                        ...
                        toast.error(
                            <div className="flex flex-col">
                                ...
                                <span className="text-xs mt-1">Contact support if coins are missing</span>
                            </div>,
                            { duration: 8000 }
                        );
                    }
    ...
    const handlePurchase = async (packageId) => {
        ...
        try {
            const response = await api.post("/api/wallet/checkout", { packageId });
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-black p-4 md:p-6">
            <div className="max-w-4xl mx-auto text-white">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-white/50 text-sm">Manage your WS-Coins</p>
                        </div>
                    </div>
                    <button
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-4xl font-bold">
                                {wallet.balance.toLocaleString()} <span className="text-xl text-yellow-400">WS-Coins</span>
                            </p>
                            <p className="text-white/50 text-sm mt-1">≈ €{(wallet.balance / 100).toFixed(2)}</p>
                        </div>

                        <div className="flex gap-6 text-sm">
                            <div className="text-center p-3 bg-green-500/10 rounded-xl">
                                <p className="text-green-400 font-bold text-xl">{wallet.totalReceived.toLocaleString()}</p>
                                <p className="text-white/50">Received</p>
                            </div>
                            <div className="text-center p-3 bg-red-500/10 rounded-xl">
                                <p className="text-red-400 font-bold text-xl">{wallet.totalSpent.toLocaleString()}</p>
                ...
                <div className="flex border-b border-white/10 mb-6 overflow-x-auto">
                    {[
                        { id: "buy", label: "Buy Coins", icon: <Icons.Coins className="w-4 h-4" />, emoji: "🪙" },
                        { id: "withdraw", label: "Withdraw", icon: <Icons.ArrowDown className="w-4 h-4" />, emoji: "💸" },
                        { id: "history", label: "History", icon: <Icons.History className="w-4 h-4" />, emoji: "📜" },
                    ].map(tab => (
                        <button
                            key={tab.id}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {COIN_PACKAGES.map((pkg) => (
                                <div
                                    key={pkg.id}
                                    className={`relative bg-white/5 border rounded-2xl p-5 transition hover:bg-white/10 hover:scale-[1.02] ${pkg.popular ? "border-cyan-500 ring-2 ring-cyan-500/50" : "border-white/10"
                                        }`}
                                >
                                    {pkg.popular && (
                                    ...
                                    <div className="text-center mb-4">
                                        ...
                                        <p className="text-3xl font-bold text-yellow-400 mt-2">{pkg.coins.toLocaleString()}</p>
                                        <p className="text-white/50">WS-Coins</p>
                        ...
                        <div className="mt-8 p-4 bg-white/5 rounded-xl border border-white/10">
                            <p className="text-white/50 text-sm mb-3">🔒 Secure payments powered by Stripe</p>
                    ...
                    <div className="max-w-md">
                        ...
                        <div className="bg-white/5 p-4 rounded-xl border border-white/10 mb-6">
                            <p className="text-white/60 text-sm">
                                💡 Minimum withdrawal: <span className="text-white font-semibold">1000 coins (€10)</span>
                            </p>
                            <p className="text-white/60 text-sm mt-1">
                                Exchange rate: <span className="text-white font-semibold">100 coins = €1</span>
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleWithdraw} className="space-y-4">
                            ...
                            <div>
                                ...
                                {withdrawAmount && (
                                    <p className="text-white/50 text-sm mt-1">
                                        = €{(parseInt(withdrawAmount) / 100 || 0).toFixed(2)}
                                    </p>
                                )}
                            </div>
                            ...
                            {/* Method */}
                            <div>
                                <label className="block text-white/70 text-sm mb-2">Withdrawal Method</label>
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        ...
                                        >
                                            ...
                                            <p className="text-sm mt-1 font-medium">{method.label}</p>
                                        </button>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <label className="block text-white/70 text-sm mb-2">
                                    {withdrawMethod === "paypal" ? "PayPal Email" : "IBAN Number"}
                                </label>
                                <input
                                    type="text"
                                    value={accountDetails}
                                    onChange={(e) => setAccountDetails(e.target.value)}
                                    placeholder={withdrawMethod === "paypal" ? "your@email.com" : "NL00 BANK 0000 0000 00"}
                                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/40 focus:border-cyan-400 outline-none transition"
                                />
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={withdrawLoading || !withdrawAmount || parseInt(withdrawAmount) < 1000}
                        ...
                        <div className="mt-6 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                            <p className="text-yellow-400 text-sm font-medium mb-2">⏱️ Processing Time</p>
                    ...
                    <div>
                        ...
                        {transactions.length === 0 ? (
                            <div className="text-center py-12 bg-white/5 rounded-xl border border-white/10">
                                ...
                                <p className="text-white/50 mt-4">No transactions yet</p>
                                <p className="text-white/30 text-sm mt-1">Your transaction history will appear here</p>
                            </div>
                        ) : (
